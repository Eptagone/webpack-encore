...
function createWebpackConfig(outputDirName = '', command: string, argv: Record<string, unknown> | yargsParser.Arguments = {}) {
...
function convertToManifestPath(assetSrc: string, webpackConfig: WebpackConfig & { outputPath: string }) {
...
function readOutputFileContents(filename: string, config: WebpackConfig & { outputPath: string }) {
...
function getEntrypointData(config: WebpackConfig & { outputPath: string }, entryName: string) {
...
function getIntegrityData(config: WebpackConfig & { outputPath: string }) {
    const entrypointsData = JSON.parse(readOutputFileContents('entrypoints.json', config));
...
describe('Functional tests using webpack', function () {
    ...
    before(function (done) {
        this.timeout(15000);
        // @ts-expect-error Is 'new' really a valid value for headless?
...
`,
        ...
        it('TypeScript is compiled and type checking is done in a separate process!', (done) => {
            ...
            config.enableForkedTypeScriptTypesChecking((_config) => {
    ...
    require('autoprefixer')()
        ...
        it('Vue.js supports CSS/Sass/Less/Stylus/PostCSS modules', (done) => {
            ...
            config.configureCssLoader((options) => {
                if (options.modules && typeof options.modules !== 'string' && options.modules !== true) {
  ...
  plugins: [
    require('autoprefixer')()
            ...
            testSetup.runWebpack(config, (webpackAssert) => {
                ...
                const expectClassDeclaration = (className: string) => {
                    webpackAssert.assertOutputFileContains(
                        'main.css',
                        `.${className} {`,
                    );
                ...
                testSetup.requestTestPage(
                    ...
                    async ({ page }) => {
                        // @ts-expect-error It's okey. It's a test
                        const divClassArray = await page.evaluate((): string[] => Array.from(document.body.querySelector('#app > div').classList.values()));
  ...
  plugins: [
    require('autoprefixer')()
        ...
        it('Vue.js error when using non-activated loaders', (done) => {
            ...
            testSetup.runWebpack(config, (_webpackAssert, _stats, output) => {
    ...
    require('autoprefixer')()
        ...
        describe('copyFiles() allows to copy files and folders', () => {
            ...
            it('Do not try to copy files from an invalid path', (done) => {
                ...
                testSetup.runWebpack(config, (_webpackAssert, _stats, stdout) => {
                    expect(config.outputPath).to.be.a.directory()
                        .with.files([
                            'entrypoints.json',
                            'runtime.js',
        ...
        describe('entrypoints.json & splitChunks()', () => {
            ...
            it('Use splitChunks in production mode', (done) => {
                ...
                testSetup.runWebpack(config, () => {
                    ...
                    const entrypointsData = JSON.parse(readOutputFileContents('entrypoints.json', config));
                    const mainJsFiles: string[] = entrypointsData.entrypoints.main.js;
            ...
            it('Make sure chunkIds do not change between builds', (done) => {
                ...
                const createSimilarConfig = function (includeExtraEntry: boolean) {
                    const config = createWebpackConfig('web/build', 'production');
            ...
            it('Do not change contents or filenames when more modules require the same split contents', (done) => {
                ...
                const getSplitVendorJsPath = function (config: WebpackConfig & { outputPath: string }) {
                    const entrypointData = getEntrypointData(config, 'main3');

                    const splitFiles = entrypointData.js.filter((filename: string) => {
